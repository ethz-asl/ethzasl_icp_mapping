...
class Mapper {
  ...
  bool mapping;
  bool
      inverseTransform; // If true, publish the inverse transform, useful to maintain a tree in TF
  ...
  const double
      mapElevation; // initial correction on z-axis //FIXME: handle the full matrix
...
void Mapper::gotScan(const sensor_msgs::LaserScan &scanMsgIn) {
  if (localizing) {
    const ros::Time endScanTime(scanMsgIn.header.stamp + ros::Duration(
        scanMsgIn.time_increment * (scanMsgIn.ranges.size() - 1)));
    try {
      unique_ptr<DP> cloud
          (new DP(PointMatcher_ros::rosMsgToPointMatcherCloud<float>(scanMsgIn,
                                                                     &tfListener,
                                                                     scanMsgIn.header.frame_id))); //odomFrame
      processCloud(move(cloud),
                   scanMsgIn.header.frame_id,
                   endScanTime,
                   scanMsgIn.header.seq);
    }
